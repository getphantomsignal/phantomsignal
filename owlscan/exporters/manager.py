"""
OwlScan Export Manager — Intelligence Packet Compiler
...
from owlscan.core.config import config as owlscan_config
from owlscan.core.database import get_db
from owlscan.core.models import Export, Scan, ScanResult

logger = logging.getLogger("owlscan.exporters")
...
class ExportManager:
    ...
    def __init__(self, output_dir: Optional[str] = None):
        self.output_dir = Path(output_dir or owlscan_config.get("export", "output_dir", default="./exports"))
    ...
    def export(
        ...
        base_name = f"owlscan_{safe_target}_{timestamp}"
        output_path = self.output_dir / f"{base_name}{ext}"

        payload = {
            "owlscan_version": "1.0.0",
    ...
    def _export_xml(self, payload: Dict, path: Path) -> None:
        ...
        root = Element("OwlScanReport")
        root.set("version", payload["owlscan_version"])
    ...
    def _export_xlsx(self, payload: Dict, path: Path) -> None:
        ...
        scan = payload["scan"]
        ws_summary.append(["OwlScan Intelligence Report"])
    ...
    def _export_pdf(self, payload: Dict, path: Path) -> None:
        ...
        story.append(Paragraph("OwlScan OSINT Report", title_style))
...
<head>
<meta charset="UTF-8">
<title>OwlScan Report — {scan.get('target')}</title>
<style>
...
<body>
<h1>🦉 OwlScan Intelligence Report</h1>
<div class="meta">
  <table>
...
{results_html}
<footer>Generated by OwlScan OSINT Framework v{payload['owlscan_version']} | "See everything. Leave no trace."</footer>
</body>
</html>"""
        with open(path, "w", encoding="utf-8") as f:
            f.write(html)
...
"""
        ...
        md += "\n*Generated by [OwlScan OSINT Framework](https://github.com/owlscan/owlscan)*"
    ...
    def decrypt_file(enc_path: str, password: str, output_path: Optional[str] = None) -> str:
        """Decrypt an AES-256-GCM encrypted OwlScan export."""
        from cryptography.hazmat.primitives.ciphers.aead import AESGCM
        from cryptography.hazmat.primitives.kdf.scrypt import Scrypt
        ...
        with open(enc_path, "rb") as f:
            magic = f.read(4)
            if magic != b"NOWL":
                raise ValueError("Not a valid OwlScan encrypted file")
            salt = f.read(16)
            nonce = f.read(12)
            ciphertext = f.read()
