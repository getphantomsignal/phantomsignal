"""
NightOwl Web Interface — The Shadow Grid Control Panel
...
def create_app(config_path: Optional[str] = None) -> Flask:
    ...
    app.config["SECRET_KEY"] = nightowl_config.get("server", "secret_key")
    app.config["SQLALCHEMY_DATABASE_URI"] = nightowl_config.get("database", "url", default="sqlite:///nightowl.db")
    app.config["SQLALCHEMY_TRACK_MODIFICATIONS"] = False
    app.config["NIGHTOWL_VERSION"] = __version__
    app.config["NIGHTOWL_CODENAME"] = __codename__

    init_db()

    from nightowl.core.engine import engine as phantom_engine
    phantom_engine._socketio = socketio
    app.phantom_engine = phantom_engine

    socketio.init_app(app)

    from nightowl.web.routes.dashboard import dashboard_bp
    from nightowl.web.routes.scans import scans_bp
    from nightowl.web.routes.intel import intel_bp
    from nightowl.web.routes.settings import settings_bp
    from nightowl.web.routes.export import export_bp
    from nightowl.web.routes.api import api_bp
